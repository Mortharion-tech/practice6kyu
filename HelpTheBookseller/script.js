...
function stockList(books, categories) {
    ...
    const splitBook = books.map((book)=>book.split(' '));
  //  return splitBook;
  //  [ [ 'BBAR', '150' ], [ 'CDXE', '515' ], [ 'BKWR', '250' ], [ 'BTSQ', '890' ], [ 'DRTY', '600' ] ]
    const charAmounts = splitBook.map((innerArr)=>innerArr.map((book, ind)=> {
      if (ind === 0) {
        return book.charAt(0);
      } else {
        return book;
      }
    })); 
  //  return charAmounts;
  //  [ [ 'B', '150' ], [ 'C', '515' ], [ 'B', '250' ], [ 'B', '890' ], [ 'D', '600' ] ]
    const amountArr = categories.map((val)=> {
      return val + " : " + charAmounts.filter(([char, num])=>char.startsWith(val)).reduce((sum, [alph, num])=>sum + Number(num), 0);
      });
  //  return amountArr;
  //  [ 'A : 0', 'B : 1290', 'C : 515', 'D : 600' ]
    return amountArr.map(item => `(${item})`).join(' - ');
}
