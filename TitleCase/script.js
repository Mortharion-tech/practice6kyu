...
function titleCase(title, minorWords) {
    const minors = minorWords ? minorWords.toLowerCase().split(' ') : [];
    return title.split(' ').map((word, ind)=> {
      if (ind === 0) {
        return word.slice(0,1).toUpperCase() + word.slice(1).toLowerCase();
      } else if (minorWords && minors.includes(word.toLowerCase())) {
...
/* function titleCase(title, minorWords) {
    const minors = minorWords ? new Set(minorWords.toLowerCase().split(' ')) : new Set();
    return title.toLowerCase().split(' ').map((word, ind) => {
      if (word) {
        if (ind === 0 || !minors.has(word)) {
