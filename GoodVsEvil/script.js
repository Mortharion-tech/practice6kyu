...
function goodVsEvil(good, evil) {
    ...
    const goodPow = good.split(' ').map((val, ind)=> {
      return goodMap[ind] * val;
    }).reduce((sum, val)=>sum+val, 0);
    const evilPow = evil.split(' ').map((val, ind)=> {
      return evilMap[ind] * val;
    }).reduce((sum,val)=>sum+val, 0);
    return goodPow > evilPow ? "Battle Result: Good triumphs over Evil" : evilPow > goodPow ? "Battle Result: Evil eradicates all trace of Good" : "Battle Result: No victor on this battle field";
}