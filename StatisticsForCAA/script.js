...
function parseTime(timeStr) {
  ...
  function stat(strg) {
    if (!strg) return "";
    const timesInSeconds = strg.split(',').map(time => parseTime(time.trim())).sort((a, b) => a - b);
    const rangeInSeconds = timesInSeconds[timesInSeconds.length - 1] - timesInSeconds[0];
    const sumInSeconds = timesInSeconds.reduce((acc, time) => acc + time, 0);
    const averageInSeconds = sumInSeconds / timesInSeconds.length;
    const medianInSeconds = timesInSeconds.length % 2 === 0
    ...
    const range = formatTime(rangeInSeconds);
